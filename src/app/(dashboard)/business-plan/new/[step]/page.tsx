...
export default async function BusinessPlanStepPage({ params, searchParams }: PageProps) {
  const { step } = await params;
  const { planId } = await searchParams;
  const stepNumber = parseInt(step, 10);
  const totalSteps = businessPlanQuestionnaire.steps.length;

  // Validate step number
  if (isNaN(stepNumber) || stepNumber < 1 || stepNumber > totalSteps) {
    redirect('/business-plan/new/1');
  }
  ...
  if (!currentPlanId) {
    ...
    redirect(`/business-plan/new/${step}?planId=${currentPlanId}`);
  }
